...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
                          <div class="card-body">
                              <h4 class="card-title">ASSIGN PHRASE</h4>
                                        ...
                                        <div class="col-12">
                                          ...
                                          <select class="form-control select2-container input-lg step2-select" id="phrases" name="states[]" multiple="multiple">
                                            @foreach($phrases as $p)
                                              <option value="{{$p->id}}" >{{$p->text_spanish}}</option>
                                            @endforeach
                                          </select>
          ...
          <div class="card">
              ...
              <div class="card-body">
                  ...
                  <div class="d-flex justify-content-between mb-2">
                    <div>
                      <h4 class="card-title">PREVIOUS PHRASE ASSIGNMENTS</h4>
...
<script>
  $(document).ready(function(){
    ...
    $("#submit-form").on('click',function(){
      ...
      if(user != '' && phrases != '')
      {
        if(phrases.length >= 5)
        {
          ...
          fn.append("user", user)
          fn.append("phrases", JSON.stringify(phrases))
          $.ajax({
            type:'POST',
            url:'{{url("/admin/phrase/assign")}}',
            data: fn,
            contentType: false,
                      cache: false,
                      processData: false,
    ...
    $("#selectAttempt").on('change',function(){
      ...
      const user = $("#user-id").val()
      window.location.href = "{{url('')}}/admin/phrase/assign/view-attempt/"+user+"/"+attempt_no
    })
  })
</script>
@stop
